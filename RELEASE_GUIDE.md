# Table-Remodeler release guide

This document provides step-by-step instructions for releasing a new version of table-remodeler to PyPI.

**Platform support:** This guide includes commands for both Windows (PowerShell) and Linux/macOS (Bash).

## Prerequisites

Before starting the release process, ensure you have:

- [ ] Write access to the table-remodeler repository (maintainer/collaborator)
- [ ] PyPI account with maintainer access to the `table-remodeler` package
- [ ] Git configured with your credentials
- [ ] Python 3.10+ installed
- [ ] Virtual environment activated (if using one)
- [ ] Your fork of table-remodeler set up as `origin` remote
- [ ] hed-standard/table-remodeler set up as `upstream` remote
- [ ] All intended changes merged to the `main` branch of upstream

**Git remote setup:**

If you haven't set up your remotes, run:

```bash
# Check your current remotes
git remote -v

# If needed, add upstream (assuming origin is your fork)
git remote add upstream https://github.com/hed-standard/table-remodeler.git
```

Your setup should look like:

- `origin` → your fork (e.g., `yourusername/table-remodeler`)
- `upstream` → `hed-standard/table-remodeler`

## Release checklist

### 1. Pre-release preparation

#### 1.1 Ensure working tree is clean

**Windows (PowerShell):**

```powershell
# Check git status
git status

# If there are uncommitted changes, commit or stash them
git add <files>
git commit -m "Pre-release cleanup"
```

**Linux/macOS (Bash):**

```bash
# Check git status
git status

# If there are uncommitted changes, commit or stash them
git add .
git commit -m "Pre-release cleanup"
```

#### 1.2 Update CHANGELOG.md

Add a new entry at the top of `CHANGELOG.md` with:

- Release version number (e.g., "Release <version>")
- Release date
- Bullet points describing:
  - New features
  - Enhancements
  - Bug fixes
  - Documentation improvements
  - Breaking changes (if any)

**Recommend using AI assistance to produce a draft change log reflecting changes between the last release and the current commit on the main branch.**

**Example:**

```markdown
Release <version> <date>
- Applied ruff code formatter to entire codebase for consistent code style
- Enhanced code quality with ruff linting and formatting
- Enhanced CONTRIBUTING.md with code formatting guidelines
- Updated README.md with ruff usage instructions
```

#### 1.3 Run code quality checks

Before releasing, ensure all code quality checks pass:

**All platforms:**

```bash
# Run code formatter check
ruff format --check .

# Run linter
ruff check remodeler/ tests/

# Run spell checker
typos

# Run all tests
python -m unittest discover tests -v
```

Fix any issues before proceeding.

#### 1.4 Commit CHANGELOG updates

**All Platforms:**

```bash
git add CHANGELOG.md
git commit -m "Update CHANGELOG for version <version>"
```

#### 1.5 Push CHANGELOG to your fork and create pull request

Push your changes to your fork and create a pull request to upstream:

**All Platforms:**

```bash
# Push to your fork (origin)
git push origin your-feature-branch

# OR if you're working directly on main in your fork:
git push origin main
```

**Create pull request:**

1. Go to your fork on GitHub: `https://github.com/yourusername/table-remodeler`
2. Click **"Contribute"** → **"Open pull request"**
3. Target: `hed-standard/table-remodeler` (base: `main`)
4. Source: Your fork and branch
5. Title: "Update CHANGELOG for version <version>" (or similar)
6. Create and merge the pull request

**Important:** Ensure the CHANGELOG updates are merged into the upstream `main` branch (hed-standard/table-remodeler) before proceeding.

### 2. Create GitHub release

The project uses [setuptools-scm](https://github.com/pypa/setuptools_scm) for version management, which automatically derives the version from git tags. We'll create the release tag and release directly on GitHub.

#### 2.1 Navigate to create release

Go to: https://github.com/hed-standard/table-remodeler/releases/new

#### 2.2 Create release tag and release

Fill in the release form:

1. **Choose a tag:** Type a new tag: `<version>`

   - **Target:** main branch
   - Use semantic versioning: MAJOR.MINOR.PATCH
   - Do NOT use a prefix (e.g., use `1.1.1`, not `v1.1.1`)

2. **Release title:** `Release <version>`

3. **Description:** Paste the detailed changelog information generated by Copilot

**Example description:**

```markdown
## What's New in <version>

### Features
- [List key features added in this release]

### Documentation
- [List documentation improvements]

### Bug Fixes
- [List bug fixes]

### Full Changelog
See [CHANGELOG.md](https://github.com/hed-standard/table-remodeler/blob/main/CHANGELOG.md)
```

4. **Set as latest release:** Check this box

5. Click **Publish release**

#### 2.3 Verify GitHub release

Verify the release on GitHub:

- [ ] Release appears at https://github.com/hed-standard/table-remodeler/releases
- [ ] Tag `<version>` was created
- [ ] Release notes display correctly
- [ ] Target is set to main branch

### 3. Verify Zenodo release

Zenodo automatically creates a DOI and archives the release when a GitHub release is published. This happens through the GitHub-Zenodo integration.

#### 3.1 Check Zenodo release

**Wait a few minutes** after creating the GitHub release for Zenodo to process it.

1. Go to: https://zenodo.org/search?q=table-remodeler (or the latest Zenodo record for table-remodeler)
2. Verify that a new version appears for your release
3. Check that the DOI is generated
4. Confirm the metadata is correct (title, authors, description)

**Note:** If the Zenodo release doesn't appear after 10-15 minutes, check the GitHub-Zenodo integration settings or contact a repository maintainer.

### 4. Pull release tag to local repository

Now that the release is created on GitHub and verified on Zenodo, pull the tag to your local repository.

#### 4.1 Update local repository from upstream

**All Platforms:**

```bash
# Switch to main branch
git checkout main

# Pull the latest changes from upstream (hed-standard/table-remodeler)
git pull upstream main

# Fetch all tags from upstream
git fetch upstream --tags

# Verify the tag exists locally
git tag -l <version>

# Optional: Update your fork's main branch
git push origin main
```

#### 4.2 Verify version

**All Platforms:**

```bash
# Check that the version is correctly detected
python -c "import remodeler; print(remodeler.__version__)"
```

Expected output: `<version>`

If you see something like `0+untagged.xxx.gxxxxxxx`, ensure:

- You're on the main branch
- The tag was properly fetched: `git fetch --tags`
- The tag exists: `git tag -l`

### 5. Build distribution packages

#### 5.1 Clean previous builds

**Windows (PowerShell):**

```powershell
# Remove old build artifacts
Remove-Item -Recurse -Force dist, build, *.egg-info -ErrorAction SilentlyContinue
```

**Linux/macOS (Bash):**

```bash
# Remove old build artifacts
rm -rf dist build *.egg-info
```

#### 5.2 Install/upgrade build tools

**All Platforms:**

```bash
python -m pip install --upgrade build twine
```

#### 5.3 Build the packages

**All Platforms:**

```bash
# Build both wheel and source distributions
python -m build
```

This creates:

- `dist/table_remodeler-<version>-py3-none-any.whl` (wheel distribution)
- `dist/table_remodeler-<version>.tar.gz` (source distribution)

#### 5.4 Verify build contents

**Windows (PowerShell):**

```powershell
# List contents of the wheel
python -m zipfile -l dist/table_remodeler-<version>-py3-none-any.whl

# Check the source distribution (requires tar in PATH)
tar -tzf dist/table_remodeler-<version>.tar.gz
```

**Linux/macOS (Bash):**

```bash
# List contents of the wheel
unzip -l dist/table_remodeler-<version>-py3-none-any.whl

# Check the source distribution
tar -tzf dist/table_remodeler-<version>.tar.gz
```

### 6. Test the distribution (recommended)

#### 6.1 Create test environment

**Windows (PowerShell):**

```powershell
# Create a fresh virtual environment for testing
python -m venv test_env
.\test_env\Scripts\Activate.ps1
```

**Linux/macOS (Bash):**

```bash
# Create a fresh virtual environment for testing
python -m venv test_env
source test_env/bin/activate
```

#### 6.2 Install from wheel

**All Platforms:**

```bash
# Install the built wheel
pip install dist/table_remodeler-<version>-py3-none-any.whl
```

#### 6.3 Run tests

**Windows (PowerShell):**

```powershell
# Navigate back to the repository
cd <path-to-table-remodeler-repo>

# Run the test suite
python -m unittest discover tests -v
```

**Linux/macOS (Bash):**

```bash
# Navigate back to the repository
cd <path/to/table-remodeler>

# Run the test suite
python -m unittest discover tests -v
```

#### 6.4 Verify installation

**All Platforms:**

```bash
# Check version
python -c "import remodeler; print(remodeler.__version__)"

# Test a basic import
python -c "from remodeler import Dispatcher; print('Success!')"
```

#### 6.5 Clean up test environment

**Windows (PowerShell):**

```powershell
deactivate
Remove-Item -Recurse -Force test_env
```

**Linux/macOS (Bash):**

```bash
deactivate
rm -rf test_env
```

### 7. Upload to PyPI

#### 7.1 Check distribution with twine

**All Platforms:**

```bash
# Validate the distribution packages
python -m twine check dist/*
```

Expected output: `Checking dist/table_remodeler-<version>.tar.gz: PASSED` (and same for .whl).

#### 7.2 Upload to test PyPI (optional but recommended)

**All Platforms:**

```bash
# Upload to TestPyPI first
python -m twine upload --repository testpypi dist/*
```

You'll be prompted for your TestPyPI credentials.

**Test installation from TestPyPI:**

```bash
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ table-remodeler==<version>
```

#### 7.3 Upload to production PyPI

**All Platforms:**

```bash
# Upload to the real PyPI
python -m twine upload dist/*
```

You'll be prompted for your PyPI credentials (or use an API token).

**Alternative with API Token:**

*Windows (PowerShell):*

```powershell
# Set your PyPI API token as an environment variable
$env:TWINE_USERNAME = "__token__"
$env:TWINE_PASSWORD = "pypi-your-api-token-here"

# Upload
python -m twine upload dist/*
```

*Linux/macOS (Bash):*

```bash
# Set your PyPI API token as an environment variable
export TWINE_USERNAME="__token__"
export TWINE_PASSWORD="pypi-your-api-token-here"

# Upload
python -m twine upload dist/*
```

#### 7.4 Verify on PyPI

Visit https://pypi.org/project/table-remodeler/ and verify:

- [ ] The new version is listed.
- [ ] README renders correctly
- [ ] All metadata is correct
- [ ] Download links work

### 8. Attach build artifacts to GitHub release (optional)

You may want to attach the distribution files to the GitHub release for convenience.

#### 8.1 Navigate to release

Go to: https://github.com/hed-standard/table-remodeler/releases/tag/<version>

#### 8.2 Edit release and attach files

- Click **Edit release**
- Drag and drop or upload:
  - `dist/table_remodeler-<version>-py3-none-any.whl`
  - `dist/table_remodeler-<version>.tar.gz`
- Click **Update release**

### 9. Post-release verification

#### 9.1 Test installation from PyPI

In a fresh environment:

**All Platforms:**

```bash
pip install --upgrade table-remodeler
python -c "import remodeler; print(remodeler.__version__)"
```

Expected output: `<version>`

#### 9.2 Verify documentation

Check that documentation sites are updated (may take some time):

- https://www.hedtags.org/table-remodeler/

#### 9.3 Announce the release

Consider announcing the release:

- [ ] GitHub Discussions
- [ ] Community forums
- [ ] Social media (if applicable)

### 10. Troubleshooting

#### Issue: Wrong version number after tagging

**Problem:** `python -c "import remodeler; print(remodeler.__version__)"` shows wrong version

**Solution:**

```bash
# Fetch all tags from upstream (not from your fork)
git fetch upstream --tags

# Verify tag exists
git tag -l

# Check setuptools-scm can find the version
python -m setuptools_scm
```

#### Issue: Build fails with "No module named X"

**Problem:** Missing dependencies during build

**Solution:**

```bash
# Ensure build tools are installed
pip install --upgrade pip setuptools wheel build

# Try building again
python -m build
```

#### Issue: Upload to PyPI fails with authentication error

**Problem:** Invalid credentials or API token

**Solution:**

- Create a new API token at https://pypi.org/manage/account/token/
- Scope it to the `table-remodeler` project only
- Use `__token__` as username and the token as password

#### Issue: Tag already exists

**Problem:** You need to delete and recreate a tag

**Solution:**

**Note:** Tags are created on upstream (hed-standard/table-remodeler) via GitHub releases. If you need to delete a tag:

```bash
# Delete local tag
git tag -d <version>
```

**To delete from upstream (requires maintainer access):**

1. Go to https://github.com/hed-standard/table-remodeler/releases
2. Delete the release associated with the tag
3. Delete the tag: Settings → (or use GitHub API/CLI)

OR use git (if you have push access to upstream):

```bash
# Delete remote tag from upstream (CAUTION: only if not yet released!)
git push upstream :refs/tags/<version>
```

**WARNING:** Only delete remote tags if the release hasn't been published to PyPI yet!

## Version Numbering Guidelines

Table-Remodeler follows [Semantic Versioning](https://semver.org/):

- **MAJOR version (X.0.0):** Incompatible API changes
- **MINOR version (0.X.0):** New features, backward-compatible
- **PATCH version (0.0.X):** Bug fixes, backward-compatible

### When to increment:

- **MAJOR:** Breaking changes, API redesign, major architectural changes
- **MINOR:** New features, new functions/classes, enhancements (this release: e.g., 0.9.0)
- **PATCH:** Bug fixes, documentation fixes, small improvements (e.g., 0.9.1)

## Quick Reference Commands

**New workflow summary:**

1. Update CHANGELOG and push to your fork (origin)
2. Create PR to upstream and merge to hed-standard/table-remodeler main
3. Create release tag and release on upstream GitHub (hed-standard/table-remodeler)
4. Verify Zenodo release (automatic, linked to GitHub)
5. Pull tag from upstream to local repository
6. Build distributions and upload to PyPI

**Important:**

- `origin` = your fork
- `upstream` = hed-standard/table-remodeler

**Windows (PowerShell):**

```powershell
# Step 1: Update and push CHANGELOG to your fork
git status                                    # Check working tree
git add CHANGELOG.md                          # Stage changelog
git commit -m "Update CHANGELOG for version <version>"   # Commit
git push origin your-branch                   # Push to your fork

# Step 2: Create PR on GitHub (via web interface)
# - Go to your fork on GitHub
# - Create PR to hed-standard/table-remodeler main
# - Merge the PR

# Step 3: Create release on upstream GitHub (via web interface)
# - Go to https://github.com/hed-standard/table-remodeler/releases/new
# - Create tag <version> targeting main
# - Add release notes (paste Copilot-generated changelog)
# - Publish release

# Step 4: Verify Zenodo release (via web interface)
# - Wait a few minutes for Zenodo to process the GitHub release
# - Check the table-remodeler Zenodo records
# - Verify new version appears with DOI

# Step 5: Pull tag from upstream to local
git checkout main                             # Switch to main
git pull upstream main                        # Pull from upstream
git fetch upstream --tags                     # Fetch tags from upstream
python -c "import remodeler; print(remodeler.__version__)" # Verify version

# Step 6: Build and upload to PyPI
Remove-Item -Recurse -Force dist, build, *.egg-info -ErrorAction SilentlyContinue
python -m build                               # Build
python -m twine check dist/*                  # Verify
python -m twine upload dist/*                 # Upload to PyPI
```

**Linux/macOS (Bash):**

```bash
# Step 1: Update and push CHANGELOG to your fork
git status                                    # Check working tree
git add CHANGELOG.md                          # Stage changelog
git commit -m "Update CHANGELOG for <version>"    # Commit
git push origin your-branch                   # Push to your fork

# Step 2: Create PR on GitHub (via web interface)
# - Go to your fork on GitHub
# - Create PR to hed-standard/table-remodeler main
# - Merge the PR

# Step 3: Create release on upstream GitHub (via web interface)
# - Go to https://github.com/hed-standard/table-remodeler/releases/new
# - Create tag <version> targeting main
# - Add release notes (paste Copilot-generated changelog)
# - Publish release

# Step 4: Verify Zenodo release (via web interface)
# - Wait a few minutes for Zenodo to process the GitHub release
# - Check the table-remodeler Zenodo records
# - Verify new version appears with DOI

# Step 5: Pull tag from upstream to local
git checkout main                             # Switch to main
git pull upstream main                        # Pull from upstream
git fetch upstream --tags                     # Fetch tags from upstream
python -c "import remodeler; print(remodeler.__version__)" # Verify version

# Step 6: Build and upload to PyPI
rm -rf dist build *.egg-info                  # Clean old builds
python -m build                               # Build
python -m twine check dist/*                  # Verify
python -m twine upload dist/*                 # Upload
```

## Additional Resources

- [Python Packaging Guide](https://packaging.python.org/)
- [setuptools-scm Documentation](https://github.com/pypa/setuptools_scm)
- [PyPI Help](https://pypi.org/help/)
- [Semantic Versioning](https://semver.org/)
- [Table-Remodeler Documentation](https://www.hedtags.org/table-remodeler/)

## Contacts

For questions about the release process:

- **Maintainer:** Kay Robbins (Kay.Robbins@utsa.edu)
- **Repository:** https://github.com/hed-standard/table-remodeler
- **Issues:** https://github.com/hed-standard/table-remodeler/issues
